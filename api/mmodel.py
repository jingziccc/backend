...
async def create_model(modelfile: UploadFile, name: str = Form(), style: str = Form(), status: str = Form(), description: str = Form(), _: User = Depends(get_current_user)):
    ...
    md5 = await get_file_md5(modelfile)
    if await MModel.exists(md5=md5):
        raise HTTPException(status_code=400, detail="模型文件已存在")
    model = await MModel.create(name=name, style=style, status=status, description=description, modelfile=await modelfile.read(), md5=md5)
    # 将模型文件存储到本地
    # 如果目录不存在则创建
